...
## Power System & RES Specialization

When reviewing papers in Electrical Engineering, Power Systems, or Renewable Energy Sources (RES), strictly enforce these methodological criteria:
- **Mathematical Rigor & Optimization:** Check the exactness of Second-Order Cone Programming (SOCP) relaxations. If Alternating Direction Method of Multipliers (ADMM) is used, rigorously verify its convergence proofs and parameter selection.
- **Control Methods:** For Model Predictive Control (MPC) and other algorithms, scrutinize the prediction horizon, stability proofs, and computational delay assumptions.
- **Modeling of RES Intermittency:** Verify that the probabilistic distributions (e.g., Weibull for wind, Beta for solar) or stochastic scenarios accurately capture the uncertainty and volatility of RES. 
- **Simulation & Power Flow:** Ensure that power flow (PF) or optimal power flow (OPF) algorithms guarantee convergence and accurately handle reactive power (PQ). Scrutinize the mathematical validity of approximations (e.g., DC power flow vs. AC power flow).
