...
The benchmark analysis, visually supported by Figure \ref{fig:cost_vs_shedding}, MPC framework:
...
ATC algorithm. The monolithic Perfect Foresight model, which processes the entire 24-hour horizon simultaneously, incurs a high computational burden, requiring a total CPU time of 584 seconds. In contrast, MPC approach distributes the computational load, resulting in a reduced total CPU time of 413 seconds. 
...
This transmission loading is most pronounced when the disturbance is extended temporally. In the Long Fault scenarios (2-MG Long and 3-MG Long), the significant active power transfer is sustained over an 11-hour window ($t=9$ to $t=19$). To mitigate the energy deficit, the decentralized algorithm re-routes power, effectively pushing the physical tie-lines toward their thermal limits. This maximized active power routing serves as the primary mechanism for load preservation, delivering available renewable generation to deficit nodes. Nevertheless, this transmission behavior acts as the foundational cause for subsequent physical consequences: the high magnitude of active power transferred across the tie-lines leads to a proportional rise in transmission currents ($I$), contributing directly to the reactive power losses ($I^2X$) and voltage degradation analyzed in the subsequent section.

\subsubsection{Voltage Stability Envelopes under Severe Grid Contingencies}

As established in the preceding section, the intensive routing of active power across the multi-microgrid network inherently induces substantial transmission currents ($I$). This large-scale power transfer generates significant physical voltage drops across the distribution feeders, governed by the impedance relationship ($I \times Z$). Consequently, the system's global voltage envelope undergoes significant degradation during the contingency windows. As illustrated in Figure \ref{fig:stage6_voltage}, this physical strain forces the voltage trajectories to exceed the predefined soft operational boundaries. However, the AC-OPF algorithm enforces a strict mathematical containment, constraining the minimum nodal voltages to the strict statutory lower bound of $0.90$ p.u., thereby preventing cascading voltage collapse under all tested extreme conditions.
...
This strict adherence to the $0.90$ p.u. threshold highlights the fundamental constraint of the physical network. Once local reactive power capacities are exhausted, the transmission infrastructure cannot sustain further power wheeling without violating the hard voltage limit. Therefore, to protect this strict stability boundary, the energy management system has no alternative mechanism remaining. The algorithm is mathematically constrained to execute targeted active load shedding to maintain system reliability and alleviate the current burden, a mechanism critically analyzed in the subsequent section.

\subsubsection{Graceful Degradation: Autonomous Load Prioritization vs. Economic Cost}

As analyzed in Section 5.4.3, the active load shedding observed across the stress matrix is driven by a dual-mechanism. In the high-transfer 2-MG Long scenario, shedding is predominantly voltage-induced, acting as a physical necessity to reduce $I \times Z$ drops and clamp the global voltage envelope at the strict 0.90 p.u. threshold. Conversely, in the highly generation-deficient 3-MG scenarios, the shedding is primarily generation-induced, executed to balance the absolute active power deficit caused by the loss of MG1. Figure \ref{fig:stage6_bar_cost} illustrates the significant escalation in True Economic Cost and Normal Load Shedding as the contingency expands spatiotemporally. When the disturbance spreads spatially from a 2-MG to a 3-MG deficit, and temporally from a Short to a Long fault duration, both the associated economic penalties and the volume of curtailed energy exhibit a significant increase. To alleviate the heavy load flow constraints and prevent a cascading voltage collapse, the decentralized algorithm autonomously curtails normal loads, utilizing them as an operational buffer.
...
The temporal dynamics of this shedding behavior demonstrate the system's operational hierarchy, as depicted in Figure \ref{fig:stage6_ts_cost}. During the extended contingency window ($t=9$ to $t=19$), the algorithm executes extensive, sustained normal load curtailment blocks to continuously stabilize the network under severe power deficits. In contrast, the critical load shedding trajectory remains at 0.0 MWh across all evaluated extreme scenarios. This consistent preservation confirms the "Autonomous Prioritization" capability embedded within the algorithm, ensuring that essential infrastructure receives uninterrupted power supply regardless of the fault severity.
