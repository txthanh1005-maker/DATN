\section*{CHAPTER 5. TEMPORAL COORDINATION WITH MODEL PREDICTIVE CONTROL}
\addcontentsline{toc}{section}{\numberline{}CHAPTER 5. TEMPORAL COORDINATION WITH MODEL PREDICTIVE CONTROL}
\setcounter{section}{5}
\setcounter{subsection}{0}
\setcounter{equation}{0}
\subsection{Model Predictive Control Time Framework}

In the operational context of Multi-Microgrid systems, independent optimization at a single time instance or a fixed time frame is often inadequate to cope with unpredictable fluctuations from load demand ($P_{load,i,t}$, $Q_{load,i,t}$) and renewable energy sources ($P_{PV,i,t}^{peak}$, $P_{WT,i,t}^{peak}$), as well as unexpected events such as main grid disconnection. To address this challenge, Model Predictive Control (MPC) is implemented as the highest-level Temporal Coordination layer.
...
In this phase, the system determines the baseline charging and discharging schedules for the Battery Energy Storage Systems (BESS) using variables such as $P_{ch,i,t}$, $P_{dis,i,t}$, $u_{ch,i,t}$, and $u_{dis,i,t}$. Furthermore, it finalizes the unit commitment status of backup sources like Diesel Generators (DG) represented by $u_{DG,i,t}$ and $v_{DG,i,t}$, aiming to minimize the overall operational costs while ensuring sufficient spinning reserve capacity ($R_{DG,t}$).

\subsubsection{Real-Time Emergency MPC}

To address immediate deviations, fault clearing, component isolation, and dynamic responses during emergencies, the Real-Time Emergency MPC is activated. This phase employs a rolling horizon of $h = 5$ hours. At each time step $t \in \mathcal{T}$, the system updates the latest measurements regarding renewable generation, actual load, and the current State of Charge of the BESS ($SOC_{i,t}$). The optimization problem is continuously re-evaluated over the sliding window from $t$ to $t + h - 1$.

During emergency conditions, such as a complete disconnection from the main grid ($P_{grid,t}^{buy} = 0$, $P_{grid,t}^{sell} = 0$), this dynamic mechanism tightly integrates with the optimization algorithms to rapidly re-coordinate the active power traded via tie-lines ($P_{tie,ij,t}$) between interconnected microgrids. By continuously rolling the time horizon, the system effectively updates $SOC_{i,t}$ based on the efficiency $\eta_{BESS}$ and makes optimal decisions to dynamically handle load shedding. It strategically allocates the shedding between normal ($P_{shed,i,t}^{normal}$) and critical loads ($P_{shed,i,t}^{critical}$) or adjusts BESS and DG operations accordingly, thereby maintaining the stability and resilience of the Multi-Microgrid system under the most severe conditions.
...
\subsubsection{State of Charge Inheritance}

In the context of the Multi-Microgrid system, the most critical physical state coupling consecutive time steps is the State of Charge (SOC) of the Battery Energy Storage Systems (BESS). The SOC at the beginning of the new rolling horizon must accurately reflect the state resulting from the control actions implemented in the previous step.
...
\end{equation}
where $SOC_{i,t-1}$ represents the State of Charge of the BESS at node $i$ evaluated at the prior interval. This value serves as the initial boundary condition for the BESS constraints over the new prediction horizon. 
...
At each control iteration starting at time $t$, the energy management system retrieves the most recent forecast profiles. These predictions are structured as vectors spanning the entire prediction horizon $h$. The forecasting vectors updated at step $t$ are formally defined as follows:
...
\begin{itemize}
    ...
    \end{align}
    \item \textbf{Renewable Energy Forecasts}: Similarly, the maximum available active power from Photovoltaic (PV) systems and Wind Turbines (WT) is integrated into the control model via vectors:
    \begin{align}
...
By systematically aligning the inherited $SOC^{ini}$ and the dynamically refreshed forecast vectors $\mathbf{P}_{load,i}^{(t)}$, $\mathbf{Q}_{load,i}^{(t)}$, $\mathbf{P}_{PV,i}^{(t)}$, and $\mathbf{P}_{WT,i}^{(t)}$, the MPC optimization engine is effectively recalibrated at every iteration. This mechanism ensures that the formulated scheduling and trading decisions are robust against uncertainties and tightly bound to the true physical constraints of the distribution network.

\subsection{The 3-Mode Spatiotemporal Coordination Framework}

The continuous and stable operation of the Model Predictive Control (MPC) strategy inherently relies on a dynamic rolling mechanism that functions as a state machine. To efficiently manage computational resources while ensuring system resilience, the spatiotemporal coordination framework is explicitly structured into three distinct operating modes: Mode 1 (Normal Operation), Mode 2 (Emergency Mode), and Mode 3 (Recovery Mode).
...
In the face of extreme scenarios such as $N-2$ contingencies and main grid connection faults (resulting in complete islanding where $P_{grid,t}^{buy} = 0$ and $P_{grid,t}^{sell} = 0$), relying solely on independent spatial or temporal coordination is mathematically and physically insufficient. The system enters Mode 2 (Emergency Mode), employing a bottom-up integration strategy that intrinsically couples the ATC spatial algorithm with the MPC rolling horizon framework. This synergistic approach allows the Multi-Microgrid system to maintain rigorous operational resilience under critical conditions.

The integration mechanism is formally structured such that the MPC layer acts as the primary temporal coordinator, dictating the rolling horizon scheduling. Within each time step $t$ of the MPC's sliding predictive window $\mathcal{H}_{MPC} = \{t, t+1, \dots, t+h-1\}$, the ATC spatial algorithm, formulated as a convex Second-Order Cone Programming (SOCP) problem, is iteratively invoked. 

When a contingency occurs, the dynamic topology and physical network constraints are instantly updated. For each prospective control interval $\tau \in \mathcal{H}_{MPC}$, the MPC provides the temporal coupling via the updated State of Charge inheritance $SOC_{i,\tau}$ (Eq. \ref{eq:SOC_transition}), alongside the most recent forecasting vectors for load demand and renewable generation ($\mathbf{P}_{load,i}^{(t)}, \mathbf{Q}_{load,i}^{(t)}, \mathbf{P}_{PV,i}^{(t)}, \mathbf{P}_{WT,i}^{(t)}$). The ATC algorithm then iteratively optimizes the spatial power distribution and active power traded via inter-microgrid tie-lines ($P_{tie,ij,\tau}$) by minimizing the ATC Global Objective (Eq. \ref{eq:ATC_Global_Objective}). The distributed coordination mathematically relies on updating the Augmented Lagrangian multiplier $\lambda_{ij,\tau}$ and the penalty parameter $\rho_{ij,\tau}$ to reach an optimal consensus on the target traded active power $P_{target,ij,\tau}$ designated by the DSO. At each iteration $k$, the multiplier update step is executed as:
...
Furthermore, to maintain energy reserves for extended islanded operation, a terminal State of Charge penalty is activated. By introducing a slack variable $S_{SOC,i}$ and a penalty coefficient $\beta_{SOC}$, the MPC penalizes the deviation of the BESS SOC at the end of the predictive horizon from a secure reference level. The SOC penalty function is given by:
\begin{equation} \label{eq:MPC_SOC_penalty}
    J_{SOC} = \sum_{i \in \mathcal{N}^{BESS}} \beta_{SOC} S_{SOC,i}^2
\end{equation}
This temporal penalty works in tandem with the spatial penalty coefficient for distribution network power losses $\omega_{loss}$, preventing aggressive discharging that would leave the BESS depleted ($SOC_{i,t}$ approaching $SOC^{min}$) for subsequent intervals.

Through this cohesive integration, the ATC SOCP algorithm resolves spatial power flow conflicts while the MPC framework optimizes the inter-temporal energy buffering and load shedding deployment, enabling the Multi-Microgrid system to navigate $N-2$ contingencies and main grid disconnections seamlessly.
