...
The overarching network topology is illustrated in Figure \ref{fig:p2p_topology}. The distinctive physical characteristics and load-generation profiles of each MG are systematically detailed as follows:
...
\begin{itemize}
    \item \textbf{Microgrid 1 (MG1) - Heavy Load with Diverse Generation:} Comprising 36 nodes, MG1 operates as a heavily loaded center within the network. It features a diverse generation portfolio, integrating multiple Renewable Energy Sources (RES) including Photovoltaic (PV) systems and a Wind Turbine (WT), alongside a Battery Energy Storage System (BESS) and a Diesel Generator (DG). MG1 maintains physical P2P tie-lines with MG3 and MG4.
    
    \item \textbf{Microgrid 2 (MG2) - Solar-Only Provider:} Characterized by a 30-node radial network, MG2 is a solar-only MG, devoid of wind generation. Thanks to its concentrated PV capacity, it acts as the major energy provider to the entire P2P network during the daytime. It incorporates BESS and DG units for stability and is directly connected to MG3 and MG4.
    
    \item \textbf{Microgrid 3 (MG3) - Light Load Savior:} As the smallest network with 21 nodes, MG3 operates with relatively light load demands but possesses excess generation capabilities. This structural advantage allows MG3 to act as the primary savior for the network, frequently exporting surplus power to support neighboring MGs. It is interconnected with MG1, MG2, and MG4.
    
    \item \textbf{Microgrid 4 (MG4) - Dense Complex with High PV:} With a total of 35 nodes, MG4 is defined by both heavy load demands and high PV capacity. It contains 25 active load nodes, making it a dense industrial and residential complex with massive power consumption. However, it heavily relies on four highly distributed PV nodes. This specific configuration causes aggressive power fluctuations, as the massive consumption drastically outweighs generation during non-sunlight hours. MG4 is interconnected with MG1, MG2, and MG3 to mitigate these aggressive internal imbalances.
\end{itemize}
...
More importantly, when evaluating the overarching system resilience through the \textit{True Economic Gap}, the disparity collapses to a mere $1.69\%$. In absolute terms, the financial discrepancy between MPC and Perfect Foresight is almost entirely attributed to minor generator dispatch inefficiencies rather than failure to serve loads. This phenomenon highlights that enforcing strict physical boundaries on the Foresight model reveals the true proximity of the MPC to theoretical perfection. The short-sightedness of the rolling horizon slightly compromises optimal generation but does not degrade the energy security capability, proving that the MPC is nearly perfect in preserving resilience.
...
This section investigates the active power flow and SOC trajectories across the four microgrids. By examining the dynamic responses of the three distinct operational strategies, we uncover the behavioral mechanisms that drive the macro-economic outcomes. The analysis focuses heavily on the interplay between proactive pre-charging and reactive emergency sharing during the severe compounded fault window ($t=9$ to $t=15$).
...
It is precisely these iteration-heavy structural shocks that drive the maximum CPU time required for a single MPC step to peak at 91 seconds, as shown in Figure \ref{fig:cpu_time}. While a 91-second computation time might initially appear substantial, it is imperative to contextualize this metric within real-world operations. In a practical Energy Management System (EMS) executing on a 1-hour dispatch interval (3600 seconds), a 91-second convergence time represents merely 2.5\% of the operational window. This leaves ample time for downstream control hardware to actuate the commands. Furthermore, this CPU spike is strictly a transient phenomenon representing the absolute worst-case scenario during a structural transition. 
