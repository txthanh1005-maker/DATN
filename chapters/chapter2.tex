\section*{CHAPTER 2. SYSTEM ARCHITECTURE AND MODELING}
\addcontentsline{toc}{section}{\numberline{}CHAPTER 2. SYSTEM ARCHITECTURE AND MODELING}
...
\begin{itemize}
    \item \textbf{Global System Settings:} The MMG network is normalized using a system base apparent power ($S_{base}$) of 1 MVA. The operational scheduling is performed over a day-ahead time horizon denoted by $\mathcal{T}$, comprising 24 intervals with a discrete time step of $\Delta t = 1$ hour.
    
    \item \textbf{P2P Trading Limits:} To respect the physical thermal capacities of the interconnected cables, the active power exchanged via tie-lines ($P_{tie,i,t}$) between interconnected MGs is strictly bounded within a trading limit of $\pm 1.5$ MW (equivalent to 1.5 pu).
...
To formulate the mathematical models of the proposed MMG system, several key sets and indices are defined. The operational time horizon is discretized into a set of intervals denoted by $\mathcal{T}$. Let $\mathcal{M}$ denote the set of all MGs in the network. Within the system, the distribution network is represented by a set of nodes $\mathcal{N}$ and a set of connecting distribution lines (branches) $\mathcal{E}$. The specific sets of nodes equipped with DERs are defined as follows: $\mathcal{N}^{PV}$ for PV systems, $\mathcal{N}^{WT}$ for WTs, $\mathcal{N}^{DG}$ for DGs, and $\mathcal{N}^{BESS}$ for BESS. 

Furthermore, the set of boundary nodes that possess tie-lines enabling physical P2P active power trading with neighboring MGs is denoted by $\mathcal{N}^{trade}$. For the radial topology of the distribution network, $\Omega(i)$ represents the set of all child nodes directly connected to a parent node $i$, and $\mathcal{C}$ defines the set of available line configurations. Throughout the formulation, subscripts $i$ and $j$ refer to network node or branch indices, whereas the subscript $t$ refers to the discrete time step.

\subsection{Mathematical Modeling of Energy Components}

This section details the mathematical formulation governing the operational constraints of the various energy components deployed within the MG network \cite{Nakiganda2023_Microgrid}. Figure \ref{fig:mg_structure} illustrates the internal configuration of a typical MG node, highlighting the integration of renewable sources, energy storage, dispatchable generators, and local loads.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.85\textwidth]{chapters/Imagine/MG structure.jpg}
    \caption{Internal configuration of a typical Microgrid node. Source: Image by AllahFoto, licensed via Adobe Stock (Education License).}
    \label{fig:mg_structure}
\end{figure}

\subsubsection{RESs and Loads}
...
\begin{subequations}
\begin{align}
    0 &\le P_{PV,i,t} \le P_{PV,i,t}^{\text{peak}}, && \forall i \in \mathcal{N}^{PV}, \forall t \label{eq:pv_limit} \\
    0 &\le P_{WT,i,t} \le P_{WT,i,t}^{\text{peak}}, && \forall i \in \mathcal{N}^{WT}, \forall t \label{eq:wt_limit}
\end{align}
\end{subequations}
...
\begin{subequations}
\begin{align}
    0 \le P_{ch,i,t} &\le u_{ch,i,t} P_{BESS}^{\max}, && \forall i \in \mathcal{N}^{BESS}, \forall t \label{eq:bess_char_limit} \\
    0 \le P_{dis,i,t} &\le u_{dis,i,t} P_{BESS}^{\max}, && \forall i \in \mathcal{N}^{BESS}, \forall t \label{eq:bess_dis_limit} \\
    u_{ch,i,t} + u_{dis,i,t} &\le 1, && \forall i \in \mathcal{N}^{BESS}, \forall t \label{eq:bess_binary_limit}
\end{align}
\end{subequations}
where $P_{ch,i,t}$ and $P_{dis,i,t}$ are the charging and discharging power, respectively, bounded by the maximum capacity $P_{BESS}^{\max}$.

The SOC ($\text{SOC}$) of the battery updates dynamically across consecutive time intervals, constrained by the energy capacity ($E_{BESS}$) and the round-trip efficiency ($\eta_{BESS}$):
\begin{subequations}
\begin{align}
    \text{SOC}_{i,t} &= \text{SOC}_{i,t-1} + \frac{P_{ch,i,t}\eta_{BESS} - P_{dis,i,t}/\eta_{BESS}}{E_{BESS}}, && \forall t > 0 \label{eq:soc_update} \\
    \text{SOC}^{\min} &\le \text{SOC}_{i,t} \le \text{SOC}^{\max}, && \forall t \label{eq:soc_limit} \\
    \text{SOC}_{i,0} &= \text{SOC}_{i,24} = \text{SOC}^{\text{ini}} && \label{eq:soc_equal}
\end{align}
\end{subequations}
Equation \eqref{eq:soc_equal} ensures the energy neutrality of the BESS over a 24-hour daily cycle, setting the initial and final $\text{SOC}$ to a predefined level $\text{SOC}^{\text{ini}}$.

\subsubsection{DG}
DGs operate as controllable power sources, governed by their minimum and maximum output boundaries. The operational status of the DG is indicated by a binary variable $u_{DG,i,t}$. To ensure the technical limits of the DGs, additional constraints including reactive power limits, startup status, minimum operating times, and spinning reserve are defined as follows:
\begin{subequations}
\begin{align}
    u_{DG,i,t} P_{DG}^{\min} \le P_{DG,i,t} &\le u_{DG,i,t} P_{DG}^{\max}, && \forall i \in \mathcal{N}^{DG}, \forall t \label{eq:dg_active_limit} \\
    (P_{DG,i,t})^2 + (Q_{DG,i,t})^2 &\le (S_{DG}^{\max})^2 u_{DG,i,t}, && \forall i \in \mathcal{N}^{DG}, \forall t \label{eq:dg_apparent_limit} \\
    u_{DG,i,t} Q_{DG}^{\min} \le Q_{DG,i,t} &\le u_{DG,i,t} Q_{DG}^{\max}, && \forall i \in \mathcal{N}^{DG}, \forall t \label{eq:dg_q_limit} \\
    v_{DG,i,t} &\ge u_{DG,i,t} - u_{DG,i,t-1}, && \forall i \in \mathcal{N}^{DG}, \forall t \label{eq:dg_startup} \\
    \sum_{\tau=t}^{t+T_{up}^{DG}-1} u_{DG,i,\tau} &\ge T_{up}^{DG} v_{DG,i,t}, && \forall i \in \mathcal{N}^{DG}, \forall t \label{eq:dg_uptime} \\
    \sum_{\tau=t}^{t+T_{down}^{DG}-1} (1 - u_{DG,i,\tau}) &\ge T_{down}^{DG} (u_{DG,i,t-1} - u_{DG,i,t}), && \forall i \in \mathcal{N}^{DG}, \forall t \label{eq:dg_downtime} \\
    R_{DG,t} &= \sum_{i \in \mathcal{N}^{DG}} \left( u_{DG,i,t} P_{DG}^{\max} - P_{DG,i,t} \right), && \forall t \label{eq:dg_reserve_def} \\
    R_{DG,t} &\ge 0.2 \sum_{i \in \mathcal{N}^{DG}} P_{DG}^{\max}, && \forall t \label{eq:dg_reserve_req}
\end{align}
\end{subequations}
where $P_{DG,i,t}$ and $Q_{DG,i,t}$ are the active and reactive power of the DG, respectively, and $S_{DG}^{\max}$ represents its maximum apparent power. $v_{DG,i,t}$ denotes the startup state, and $R_{DG,t}$ represents the spinning reserve. The minimum up and down times are defined by $T_{up}^{DG}$ and $T_{down}^{DG}$.
...
The primary objective of the local MG operator is to minimize the total operational cost over the scheduling horizon $\mathcal{T}$. To generalize the optimization problem across varying grid conditions, the fundamental objective function unifies standard economic costs and load shedding penalty costs.
...
\begin{align}
    ...
    F_{DG} &= \sum_{t \in \mathcal{T}} \sum_{i \in \mathcal{N}^{DG}} \Big( \alpha_{DG} u_{DG,i,t} + (\beta_{DG} + \kappa_{DG}) P_{DG,i,t} \nonumber \\
    &\quad + \gamma_{DG} (P_{DG,i,t})^2 + C_{start,DG} v_{DG,i,t} \Big) \label{eq:cost_dg} \\
...
The power flow within the radial network of the MGs is modeled using the original non-linear Branch Flow Model (DistFlow) equations \cite{Chowdhury2023_SOCP_ThreePhase}. The active and reactive power balance, voltage drop, and branch current definitions are formulated as follows:
...
\begin{align}
    P_{line,0,t} &= P_{grid,t}^{buy} + P_{grid,t}^{sell}, && \forall t \in \mathcal{T} \label{eq:boundary_P} \\
    Q_{line,0,t} &= Q_{grid,t}, && \forall t \in \mathcal{T} \label{eq:boundary_Q}
\end{align}
\end{subequations}
...
Equation \eqref{eq:voltage_drop} governs the voltage drop along branch $(i,j)$, considering the line impedance ($R_{ij}, X_{ij}$) and branch flows. Equation \eqref{eq:current_def} represents the non-linear definition of the squared branch current. Finally, inequalities \eqref{eq:voltage_limit} and \eqref{eq:current_limit} ensure that the squared nodal voltages and branch currents remain within their secure operational limits. This exact DistFlow model is specifically chosen as it inherently captures the physical radial topology of the MGs, forming the strict prerequisite for subsequent global optimization \cite{Chowdhury2023_SOCP_ThreePhase}. However, due to the non-linear and non-convex nature of the current definition in \eqref{eq:current_def}, this problem requires convexification via SOCP to be mathematically tractable for optimization solvers.

\subsection{SOCP Relaxation and Exactness}
\label{subsec:socp_relaxation}

The selection of an appropriate OPF model involves navigating the strict trade-off between physical accuracy and computational feasibility. As illustrated in Figure \ref{fig:opf_evolution}, simplistic models like DC-OPF fail to capture reactive power and voltage dynamics, rendering them unsafe for distribution networks with high R/X ratios. Conversely, while the Full AC-OPF guarantees physical fidelity, its non-convex nature makes it computationally intractable for real-time emergency dispatch. Therefore, the SOCP relaxation is adopted as the optimal pathway, striking the perfect balance between global optimality and strict AC physics.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{chapters/Imagine/ACOPFevolution.png}
    \caption{Evolution of Optimal Power Flow models and the justification for the SOCP approach.}
    \label{fig:opf_evolution}
\end{figure}

\subsubsection{Convexification and SOCP Relaxation}
The non-linear nature of the AC-OPF fundamentally stems from the current magnitude definition in the Branch Flow Model. As originally stated in Equation \eqref{eq:current_def}, the squared branch current is formulated as:
\begin{equation}
    I_{ij,t}^2 = \frac{P_{line,ij,t}^2 + Q_{line,ij,t}^2}{V_{i,t}^2} \nonumber
\end{equation}
This equality constraint is highly non-convex. To achieve a computationally tractable convex model, the SOC relaxation transforms the non-convex feasible region into a convex cone by replacing the equality with an inequality \cite{Chowdhury2025_SOCP_Coopt}:
\begin{equation}
...
\begin{figure}[htbp]
    \centering
    \includegraphics[width=\textwidth]{chapters/Imagine/SOCP relaxation.png}
    \caption{Geometric visualization of the SOCP relaxation.}
    \label{fig:socp_cone}
\end{figure}
...
This chapter has formulated the physical and operational constraints for radial microgrids, integrating distributed energy resources (PV, wind, DG, BESS) and enforcing strict load limits alongside VOLL penalties. To ensure operational resilience, the system toggles between a normal mode for economic dispatch and an emergency mode for critical load preservation. The network is modeled using the branch flow model (DistFlow) to capture non-linear active and reactive power dynamics. To overcome non-convexity, second-order cone programming (SOCP) relaxation was introduced, transforming physical grid constraints into a convex feasible region. This convexified AC-OPF foundation establishes the mathematical prerequisite for distributed market coordination, enabling the application of the analytical target cascading (ATC) algorithm in the subsequent chapter to securely orchestrate P2P energy trading across the multi-microgrid system.
