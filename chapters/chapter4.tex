...
\node (mode2) [process, below=of decision, yshift=-0.5cm] {\textbf{Mode 2}\\(Emergency, $h=5\text{h}$)\\\textit{Activate Constraints}\\\textit{Call Spatial ATC}};
\node (mode1) [process, left=of mode2, xshift=-0.5cm] {\textbf{Mode 1}\\(Normal)\\\textit{Apply DA Schedule}};
\node (mode3) [process, right=of mode2, xshift=0.5cm] {\textbf{Mode 3}\\(Recovery, $h=25-t$)\\\textit{Call Spatial ATC}\\\textit{Update DA Baseline}};
...
By structuring the penalty weights such that $C_{shed}^{critical} \gg \beta_{SOC} > C_{shed}^{normal}$, the SOCP mathematically guarantees that flexible loads are curtailed early to preserve BESS reserves, ensuring the sustained operation of critical infrastructure. This temporal safeguard works in tandem with the spatial penalty coefficient for distribution network power losses $\omega_{loss}$, ensuring sustainable multi-interval resilience.
...
\captionof{algorithm}{Bottom-Up Spatiotemporal Coordination as a 3-Mode State Machine}\label{alg:MPC_Emergency}
...
\For{each time step $t = 1$ to $24$}
    ...
    \ElsIf{Fault detected at current hour $t$}
        ...
        \State Call Spatial ATC Algorithm to optimize spatial power flow
