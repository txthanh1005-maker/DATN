\section*{CHAPTER 4. SPATIAL COORDINATION - ANALYTICAL TARGET CASCADING}
\addcontentsline{toc}{section}{\numberline{}CHAPTER 4. SPATIAL COORDINATION - ANALYTICAL TARGET CASCADING}
\setcounter{section}{4}
\setcounter{subsection}{0}
\setcounter{equation}{0}

\subsection{Analytical Target Cascading (ATC) Overview}
The operation of multi-microgrid (Multi-MG) systems entails a complex spatial coordination problem. Traditional centralized optimization methodologies require individual Microgrids (MGs) to transmit all their local, potentially sensitive information---such as load demand, renewable generation profiles, and Battery Energy Storage System (BESS) constraints---to a central operator. This paradigm not only creates computational bottlenecks for large-scale systems but also severely compromises the privacy of independent MG operators.

To resolve these physical and informational challenges, this framework adopts Analytical Target Cascading (ATC). ATC is a distributed optimization technique originally developed for multidisciplinary design optimization, which perfectly aligns with the hierarchical structure of modern distribution networks. By decomposing the global optimal power flow problem, ATC divides the Multi-MG system into a two-level hierarchical architecture:
\begin{itemize}
    \item \textbf{Coordinator Level (Upper Level):} The Distribution System Operator (DSO) or Utility Grid serves as the central coordinator. The DSO assigns the optimal target variables $P_{target,ij,t}$ (conceptually acting as the ideal tie-line reference) and price signals to the local MGs to ensure Peer-to-Peer (P2P) consensus.
    \item \textbf{Local Microgrid Level (Lower Level):} Each individual MG operates as an independent autonomous entity. Utilizing the target signals provided by the coordinator, each MG solves its local Second-Order Cone Programming (SOCP) problem to determine its optimal internal asset dispatch and the scheduled tie-line power exchange $P_{tie,ij,t}$.
\end{itemize}
...
At the top level of the ATC hierarchy, the Coordinator (Utility Grid) focuses entirely on optimizing the Peer-to-Peer (P2P) consensus. The coordinator's optimization problem formulates the target variable $P_{target,ij,t}$ that the lower-level MGs must track, yielding the optimal target $P_{target,ij,t} = \left( P_{tie,ij,t} - P_{tie,ji,t} \right) / 2$.
...
Consequently, each autonomous MG solves this penalized Second-Order Cone Programming (SOCP) model subject to its complete set of local operational and physical constraints defined in Chapter 3. Upon convergence of its local optimization, the MG transmits only the resulting scheduled tie-line active power exchange, $P_{tie,ij,t}$, back to the Utility Grid Coordinator for the subsequent ATC iteration.

\subsection{ATC Iterative Algorithm and Convergence}
The execution of the Analytical Target Cascading (ATC) algorithm is an iterative process that alternates between solving the local MG optimization problems and updating the coordinator's target variables and penalty multipliers. The step-by-step procedure at iteration $k$ is detailed as follows:
